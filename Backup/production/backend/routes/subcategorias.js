...
const DB_FILTROS = "supermercados_normalized_vea";
const DB_CATEGORIAS = "supermercados_metadata_vea";
...
router.get("/", async (req, res) => {
  let clientFiltros;
  let clientCategorias;

  try {
    clientFiltros = await MongoClient.connect(MONGO_URI, {
      useNewUrlParser: true,
      useUnifiedTopology: true
    });

    clientCategorias = await MongoClient.connect(MONGO_URI, {
      useNewUrlParser: true,
      useUnifiedTopology: true
    });

    const filtros = await clientFiltros
      .db(DB_FILTROS)
      .collection(COLLECTION_FILTROS)
      .find({})
      .toArray();

    const categorias = await clientCategorias
      .db(DB_CATEGORIAS)
      .collection(COLLECTION_CATEGORIAS)
      .find({})
      .toArray();
    ...
    if (clientFiltros) clientFiltros.close();
    if (clientCategorias) clientCategorias.close();
  }
});
